...
def user_studies(
    ...
    def standardize_outputs(item, db_studies_transformed):
        study_info = {
            ...
            if item.ethnic_minority is not None
            else 0,
            ...
            "racial_minority": item.racial_minority
    ...
    if study_name:
        db_study = crud.get_project_by_name(db, study_name, username)
        if db_study:
            db_studies_transformed = standardize_outputs(db_study, [])
            return {"studies": db_studies_transformed}
        else:
            return JSONResponse(
                content={"studies": ["Study not found!"]}, status_code=404
            )
